...
public class WebMWriter implements Closeable {
    ...
    public WebMReader.WebMTrack[] getTracksFromSource(final int sourceIndex) throws IllegalStateException {
        if (done) {
    ...
    private Block getNextBlockFrom(final int internalTrackId) throws IOException {
        ...
        final WebMReader.SimpleBlock res = readersCluster[internalTrackId].getNextSimpleBlock();
        if (res == null) {
    ...
    private ArrayList<byte[]> makeTrackEntry(final int internalTrackId, final WebMReader.WebMTrack track) {
