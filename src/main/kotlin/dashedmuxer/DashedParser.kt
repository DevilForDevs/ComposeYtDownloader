...
class DashedParser(file: File){
    ...
    fun parseMoov(payload: ByteArray) {
        ...
        while (buffer.remaining() >= 8) {
            ...
            when (boxType) {
                0x6D766864 -> parseMvhd(buffer.slice().limit(boxSize - 8)) // "mvhd"
                0x7472616B -> parseTrak(buffer.slice().limit(boxSize - 8)) // "trak"
            }
    ...
    fun parseTrak(data: ByteBuffer) {
        while (data.remaining() >= 8) {
            ...
            when (boxType) {
                ATOM_MDIA -> parseMdia(data.slice().limit(boxSize - 8)) // "mdia"
                ATOM_TKHD->parseTkhd(data.slice().limit(boxSize - 8))//tkhd
            }
    ...
    fun parseMdia(data: ByteBuffer) {
        while (data.remaining() >= 8) {
            ...
            when (boxType) {
                0x6D646864 -> parseMdhd(data.slice().limit(boxSize - 8)) // "mdhd"
                0x68646C72 -> parseHdlr(data.slice().limit(boxSize - 8)) // "hdlr"
                0x6D696E66 -> parseMinf(data.slice().limit(boxSize - 8)) // "minf"
            }
            data.position(boxStart + boxSize)
        }
    }
    ...
    fun parseMinf(data: ByteBuffer) {
        while (data.remaining() >= 8) {
            ...
            if (boxType == 0x7374626C) { // "stbl"
                parseStbl(data.slice().limit(boxSize - 8))
            }
            data.position(boxStart + boxSize)
        }
    }
